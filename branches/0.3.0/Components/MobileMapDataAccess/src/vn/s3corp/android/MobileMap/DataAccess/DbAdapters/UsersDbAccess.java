...
public class UsersDbAccess {
    ...
    public long createUser(User user) {
        ...
        values.put(UsersDbAccess.TABLE_KEY_USER_TYPE, user.getUserType());
     ...
     * 
     * @param
     * @return Cursor over all users, null if no user could be found or
     *         retrieved
     * @author nam.tran
     * @version 0.1.0
     * 
     ...
     * @param userId
     *            The user's ID
     * @return The cursor which is positioned to matching one, null if no user
    ...
    /**
     * Get a Cursor which is positioned at the user that has the the given login
     * name.
     * 
     * @param loginname
    ...
    public boolean updateUser(User user) {
        ...
        if (User.USER_TYPE_UNKNOWN != user.getUserType()) {
            values.put(UsersDbAccess.TABLE_KEY_USER_TYPE, user.getUserType());
        }
